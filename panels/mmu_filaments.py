...
#        edit_grid.attach(self.labels['cancel'],    14, 3,  2, 1) # No room for this, but back button is good enough
    ...
    def activate(self):
        if self.spoolmanEnabled:
            ...
            for i in range(len(self._spoolman_spools)):
                self.labels['s_selector'].append_text(str(self._spoolman_spools[i]['id']) + ':' +self._spoolman_spools[i]['filament']['vendor']['name'] + ' - '+ self._spoolman_spools[i]['filament']['name'] + ' | ' + str(round(self._spoolman_spools[i]['remaining_weight'],2)) +'g remaining')
            self.labels['s_selector'].connect("changed", self.select_spoolmon)
            self.labels['s_selector'].set_entry_text_column(0)
    ...
    def select_edit(self, widget, sel_gate):
        ...
        if spool:
            self.labels['c_selector'].set_active(self._spoolman_spools.index(spool))
